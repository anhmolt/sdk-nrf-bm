...
During initialization, the module registers a vendor-specific GATT service.
...
The service also requires :kconfig:option:`CONFIG_MEMFAULT` and a configured Memfault project key (for example :kconfig:option:`CONFIG_MEMFAULT_NCS_PROJECT_KEY`).
...
* :kconfig:option:`CONFIG_BLE_MDS_TRIGGER_LOG_COLLECTION` – Periodically snapshot the Memfault RAM log buffer into the packetizer while streaming is active.
...
1. Optionally triggers Memfault log collection (if :kconfig:option:`CONFIG_BLE_MDS_TRIGGER_LOG_COLLECTION` is enabled).
